...
export default function Home() {
  ...
  // Selection states
  const [selectedNode, setSelectedNode] = useState<GraphNode | null>(mockNodes[0]);
  ...
  const handleSelectDecisionInGraph = (decisionId: string) => {
    const target = mockNodes.find(n => n.id === decisionId);
    if (target) {
  ...
  const handleFocusGraphNodes = (nodeIds: string[]) => {
    ...
    const firstNode = mockNodes.find(n => n.id === nodeIds[0]);
    if (firstNode) {

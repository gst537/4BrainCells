...
Select a prompt below or type your inquiry to trace organizational memory:`,
  ...
  const handleSendQuery = (queryText: string) => {
    ...
    setTimeout(() => {
      // Look up precalculated or fuzzy match
      const matchedKey = Object.keys(precalculatedAnswers).find(k => 
      ...
      if (matchedKey) {
        responseMessage = {
          ...precalculatedAnswers[matchedKey],
          id: `asst-${Date.now()}`
        };
      } else {
        // Dynamic fallback response demonstrating confidence gating
        responseMessage = {
          id: `asst-${Date.now()}`,
          sender: 'assistant',
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
          ...
          text: `⚠️ **Confidence Gate: Partial Evidence Match (Confidence: 42%)**

ALETHEIA analyzed your query: *"${queryText}"*.

While several historical records mention adjacent keywords, there is **no signed Decision Record or Technical Review Board minute** directly addressing this specific query in the primary ledger.

Recommended Next Steps:
1. Cross-reference with **Dr. Elena Rostova** or **Sophia Chen**.
2. Query specific repository branches via the Knowledge Graph.
3. Check unindexed archival transcripts.`,
          citations: [
            {
              id: 'c-partial',
              docTitle: 'RFC-042: Cloud Migration Feasibility Study',
              docType: 'RFC',
              snippet: 'Relevant adjacent context found in RFC-042 infrastructure appendix.',
      ...
      if (responseMessage.graphFocusNodes && responseMessage.graphFocusNodes.length > 0) {
        onFocusGraphNodes(responseMessage.graphFocusNodes);
      }
    }, 650);
  };

...
export const AppTopBar: React.FC = () => {
  ...
  return (
    <>
      {/* Easter Egg Toast */}
      {showToast && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 bg-[#121212] border border-[#00E5FF]/40 shadow-[0_0_20px_rgba(0,229,255,0.2)] text-white px-4 py-2.5 rounded-lg flex items-center gap-3 animate-in slide-in-from-top-4 fade-in duration-300">
          <Sun className="w-4 h-4 text-[#00E5FF]" />
          <span className="text-sm font-sans">Aletheia operators only work in the dark. 🕶️</span>
        </div>
      )}
      ...
      <header className="h-14 border-b border-[#242424] bg-[#121212] px-6 flex items-center justify-between shrink-0 select-none z-20">
        ...
        <div className="flex items-center gap-3 ml-4">
          ...
          <button onClick={() => setIsNotificationsOpen(true)} title="Notifications" className="p-2 rounded-lg text-[#7F8C99] hover:text-white hover:bg-[#1c1c1c] transition-colors cursor-pointer">
            <Bell className="w-4 h-4" />
          </button>

          <button onClick={handleThemeToggle} title="Theme Toggle" className="p-2 rounded-lg text-[#7F8C99] hover:text-white hover:bg-[#1c1c1c] transition-colors cursor-pointer">
